...
- VIETATO ripetersi. Prima di scrivere rileggi verbale e memoria: se un punto e gia stato detto
  (da te o da un collega), non riproporlo — dichiarati d'accordo in mezza riga e basta.
  Ogni tuo intervento deve portare ALMENO un elemento nuovo: un angolo diverso, un numero,
  un'azione concreta non ancora sul tavolo. Un intervento che ripete cose gia dette e un intervento fallito.
- Massimo 250 parole per intervento: e una riunione, non un saggio. Se Roberto chiede
  interventi brevi, stai SOTTO la lunghezza che ha chiesto: la sua richiesta vince su tutto.
- Se l'ultimo messaggio di Roberto chiede esplicitamente a un ALTRO advisor di esporre o aprire
  e quell'advisor non ha ancora parlato nel verbale, NON rispondere al suo posto: cedi la parola
  con una sola riga (es. "La parola spetta a Su: espone lui, poi commento.") e nient'altro.
- Se ti mancano dati fondamentali (quanto debito, quali scadenze, quanti clienti attivi, costi fissi mensili),
  chiedili a Roberto: nessun vero advisor decide alla cieca. Ma se li hai gia chiesti tu o un collega
  nel verbale o nella memoria, non ripetere la stessa richiesta: aggiungi valore.`;
...
// Data e conto alla rovescia nel fuso italiano: gli advisor devono sapere che
// giorno e e quanto tempo resta della finestra settembre-novembre 2026.
function dateContext() {
  ...
  const DAY = 86400000;
  const start = Date.UTC(2026, 8, 1);   // 1 settembre 2026
  const end = Date.UTC(2026, 10, 30);   // 30 novembre 2026
  let countdown;
  if (now.getTime() < start) {
    countdown = `Mancano ${Math.ceil((start - now) / DAY)} giorni all'inizio di settembre e ${Math.ceil((end - now) / DAY)} giorni alla fine di novembre.`;
  } else if (now.getTime() <= end) {
    countdown = `Siamo DENTRO la finestra decisiva: mancano ${Math.ceil((end - now) / DAY)} giorni alla fine di novembre.`;
  } else {
    countdown = 'La finestra settembre-novembre 2026 e terminata: ragiona sullo stato attuale.';
  }
  return `\n\nDATA DI OGGI: ${oggi}. ${countdown}\nTara OGNI raccomandazione sul tempo effettivamente rimasto: date e scadenze concrete (non "questa settimana" generico ma il giorno), cosa e ancora fattibile nel tempo che resta e cosa non lo e piu. Non dare mai consigli fuori calendario.`;
}
...
async function callAdvisor(apiKey, advisorId, transcript, maxTokens = 1200, memory = '') {
  ...
  // Memoria delle riunioni passate: gli advisor sono menti persistenti,
  // non stanze isolate — cio che e stato detto in altre riunioni resta con loro.
  const memoryBlock = memory && String(memory).trim()
    ? `LA TUA MEMORIA — RIUNIONI PRECEDENTI A CUI HAI PARTECIPATO (dalla piu recente):\n` +
      `Ricordi tutto questo. Se Roberto o altri advisor fanno riferimento a idee, numeri o proposte ` +
      `emerse in una riunione passata, usale come se le avessi appena riguardate nei tuoi appunti.\n\n` +
      `${String(memory).slice(0, 40000)}\n\n=== FINE MEMORIA ===\n\n`
    : '';
  let system = cfg.prompt + dateContext();
  ...
  async function request(tokens) {
    const body = JSON.stringify({
      ...
      messages: [
        {
          role: 'user',
          content: `${memoryBlock}VERBALE DELLA RIUNIONE IN CORSO:\n\n${minutes}\n\n---\n\nOra tocca a te, ${cfg.name}. Il tuo intervento:`,
        },
      ],
    });
  ...
  // Il modello puo consumare il budget "pensando" e uscire senza testo
  // (stop_reason: max_tokens): riproviamo una volta con molto piu spazio.
...
// Ordina il giro di tavolo: chi e chiamato per nome nel messaggio apre la riunione,
// gli altri seguono nell'ordine standard e commentano. "Su" e anche una preposizione
// italiana, quindi per lui serve la virgola/du punti dopo il nome o "advisor Su".
function smartOrder(text, ids) {
  const base = ROUND_ORDER.filter((id) => ids.indexOf(id) !== -1);
  if (!text) return base;
  const hits = [];
  base.forEach((id) => {
    const short = id.charAt(0).toUpperCase() + id.slice(1);
    // "Su" maiuscolo in mezzo alla frase e sempre il nome (la preposizione e minuscola);
    // a inizio frase serve virgola/due punti o un verbo da riunione per distinguerlo.
    const re = id === 'su'
      ? /(^|[\s("])Su\s*[,:;]|[a-z\u00e0\u00e8\u00e9\u00ec\u00f2\u00f9]\s+Su\b|[Aa]dvisor\s+Su\b|(^|[.!?]\s+)Su\s+(esponi|espone|spiega|apri|apre|parla|inizia|comincia|racconta|presenta|prosegui|continua|riparti|riparte)\b/
      : new RegExp('(^|[\\s(,.;:!?"\'\u2014\u2013-])' + short + '\\b');
  ...
  return named.concat(base.filter((id) => named.indexOf(id) === -1));
}

module.exports = { CTX, ADVISORS, ROUND_ORDER, callAdvisor, smartOrder };
